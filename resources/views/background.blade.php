<x-web-layout>
    <div class="max-w-screen-xl px-4 mx-auto pb-20">
        ...
        <div class="pt-24 pb-12 text-center">
            <h2 class="text-4xl font-bold text-gray-800 tracking-tight">{{ $profile->title }}</h2>
            <div class="w-24 h-1.5 mx-auto mt-4 bg-gradient-to-r from-sky-600 to-sky-400 rounded-full"></div>
            <p class="mt-4 text-gray-500 max-w-2xl mx-auto">{{ $profile->subtitle }}</p>
        </div>
        ...
        <div class="flex flex-col md:flex-row items-center gap-12 lg:gap-16">
            ...
            <div class="w-full md:w-1/2 relative group">
                ...
                <div class="relative rounded-2xl overflow-hidden shadow-lg border border-gray-100">
                    @if ($profile->image)
                        <img src="{{ Storage::url($profile->image) }}" alt="Ilustrasi PPLG"
                    ...
                    <div
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-sm font-semibold text-gray-700">{{ $profile->badge_text }}</span>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="mt-8 grid grid-cols-1 sm:grid-cols-2 gap-4">
                    @foreach ($profile->features ?? [] as $feature)
                        <div class="flex items-center gap-3 p-3 bg-sky-50 rounded-lg border border-sky-100">
                            ...
                            <span class="font-medium text-gray-700">{{ $feature['point'] }}</span>
                        </div>
                    @endforeach
                </div>
